...
class Home(Menu):
    def __init__(self, register_controller: RegisterController , connection_controller: ConnectionController, create_tombola_controller: CreateTombolaController, play_ticket_controller: PlayTicketController, get_ticket_controller):
        ...
        self.create_tombola_item = FunctionItem("Créer une tombola", CreateTombola(self, create_tombola_controller, get_ticket_controller).execute)
        self.play_ticket_item = FunctionItem("Jouer un ticket", PlayTicket(self,play_ticket_controller, None).execute)
    ...
    def draw(self):
        if self.user_connected is None:
            ...
            if self.user_connected is not None:
                self.register_item = FunctionItem("S'enregistrer", Register(self, self.register_controller).execute)
        self.items.clear()
        if self.user_connected is None:
            # Not logged in
            ...
            if self.user_connected.is_tenant:
                ...
                self.play_ticket_item = FunctionItem("Jouer un ticket", PlayTicket(self, self.play_ticket_controller,self.user_connected).execute)
